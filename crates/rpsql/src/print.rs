...
/// Display width of a cell in characters.
///
...
fn print_aligned_text(
    ...
    if opt.topt.start_table && !opt.topt.tuples_only {
        ...
        // Headers, centered (`print.c:867`).
        for (i, header) in headers.iter().enumerate() {
            out.push(b' ');
            let nbspace = max_width[i] - cell_width(header);
    ...
    for row in cells {
        for (j, cell) in row.iter().enumerate() {
            let pad = max_width[j] - cell_width(cell);
...
mod tests {
    ...
    fn a_format_this_issue_does_not_render_is_refused_not_faked() {
        ...
        assert_eq!(
            print_query(&res, &opt),
            Err(PrintError::Unsupported(PrintFormat::Csv))
        );
    }
}
