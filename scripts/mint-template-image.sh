#!/usr/bin/env bash
# Mint the committed template image, crates/rinitdb/image/template.img, the
# minted pg_control in template form, template.control, and their provenance
# manifest (NAT-381, ADR-0002).
#
...
# Commit the image, the control file and the manifest together, and say why
# on the Linear issue.
set -euo pipefail
